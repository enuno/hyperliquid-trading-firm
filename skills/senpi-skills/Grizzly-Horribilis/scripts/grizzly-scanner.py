#!/usr/bin/env python3
# Senpi GRIZZLY HORRIBILIS Scanner v1.0
# Copyright 2026 Senpi (https://senpi.ai)
# Licensed under MIT
"""GRIZZLY HORRIBILIS — BTC Conviction-Scaled Leverage Hunter.

Same BTC thesis evaluation as Grizzly v3.0. Same entry signals.
The difference: leverage scales from 7x to 40x based on conviction score.

Why "Horribilis" (the most aggressive North American bear):
- Score 8-9:   7x leverage, 15% margin (same as standard Grizzly)
- Score 10-11: 15x leverage, 15% margin
- Score 12-13: 25x leverage, 12% margin (smaller size, bigger lever)
- Score 14+:   40x leverage, 10% margin (maximum aggression, minimum size)

The thesis: when BTC SM consensus is OVERWHELMING (score 14+ means dominant
SM, strong 4H, 1H confirms, contribution surging, funding pays, OI growing,
deep consensus), the probability is high enough to justify extreme leverage.

ALWAYS protected by DSL. At 40x leverage, an 8% retrace in ROE = 0.2% price
move. The DSL will cut this FAST if it doesn't work immediately.

The experiment: does conviction-scaled leverage produce outsized returns on
the highest-conviction setups, or does BTC noise shake out even 40x positions
before they can run?
...
LEVERAGE_TIERS = [
    (14, 40, 0.10),   # Score 14+: 40x, 10% margin
    (12, 25, 0.12),   # Score 12-13: 25x, 12% margin
    (10, 15, 0.15),   # Score 10-11: 15x, 15% margin
    (8,   7, 0.15),   # Score 8-9: 7x, 15% margin (baseline)
]
...
def evaluate_btc():
    """Identical thesis evaluation to Grizzly v3.0."""
    ...
    return {"score":score,"direction":d,"reasons":reasons,"smPct":pct,
            "smTraders":traders,"priceChg4h":p4h}

def get_leverage_and_margin(score):
    """Conviction-scaled leverage. Higher score → more aggressive."""
    for min_score, leverage, margin_pct in LEVERAGE_TIERS:
        if score >= min_score:
            return leverage, margin_pct
    return 7, 0.15
...
def run():
    ...
    leverage, margin_pct = get_leverage_and_margin(thesis["score"])
    ...
    cfg.output({
        ...
        "signal":{"asset":ASSET,"direction":thesis["direction"],"score":thesis["score"],
            "mode":"HORRIBILIS","reasons":thesis["reasons"],
            "smPct":thesis["smPct"],"smTraders":thesis["smTraders"],
            "leverage":leverage,"marginPct":margin_pct},
        "entry":{"asset":ASSET,"direction":thesis["direction"],"leverage":leverage,
            "margin":margin,"orderType":"FEE_OPTIMIZED_LIMIT"},
        "constraints":{"maxPositions":MAX_POSITIONS,"maxLeverage":40,
            "maxDailyEntries":MAX_DAILY_ENTRIES,"cooldownMinutes":COOLDOWN_MINUTES,
            "_v2_no_thesis_exit":True,
            "_note":f"HORRIBILIS: {leverage}x leverage at score {thesis['score']}. DSL ALWAYS protects."},
        "_grizzly_version":"horribilis-1.0",
    })
